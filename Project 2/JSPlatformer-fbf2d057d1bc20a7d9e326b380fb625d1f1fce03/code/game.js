  ...
  for (var y = 0; y < this.height; y++) {
    ...
    for (var x = 0; x < this.width; x++) {
      ...
      // Because there is a third case (space ' '), use an "else if" instead of "else"
        ...
        fieldType = "lava";
	//this placeholder will shrink the character
	 else if (ch == "s")
        fieldType = "shrink";
	//this placeholder will grow the character
	else if (ch == "g")
        fieldType = "grow";
...
//this is where laser can go left, right, up and down
var canShift = true;
Laser.prototype.act = function(step, level) {
  var newPos = this.pos.plus(this.speed.times(step));
  if (laserNum == 0 && canShift){
		this.speed = new Vector(3,0);
		laserNum = 1;
		canShift = false;
	}
	else if (laserNum == 1 && canShift){
		this.speed = new Vector(0,3);
		laserNum = 2;
		canShift = false;
	}
	else if (laserNum == 2 && canShift){
		this.speed = new Vector(0,-3);
		laserNum = 3;
		canShift = false;
	}
	else if (laserNum >= 3 && canShift){
		this.speed = new Vector(-3,0);
		laserNum = 0;
		canShift = false;
	}
  ...
  this.pos = this.repeatPos; canShift = true;}
  else
    this.speed = this.speed.times(-1);
};
...
Player.prototype.act = function(step, level, keys) {
  ...
  if (willShrink){
	  ...
	  this.pos = this.pos.plus(new Vector(0,-3));
  }
  if (willGrow){
	  willGrow = false;
	  isGrown = true;
...
//setting up boolean variables to make player shrink and grow
var willShrink = false;
var willGrow = false;
var isShrunk = false;
var isGrown = true;
Level.prototype.playerTouched = function(type, actor) {

//Check if player is grown or shrunk and change accordingly
  if (type == "shrink" && isGrown){
	  willShrink = true;
  }
  else if (type == "grow" && isShrunk){
	  willGrow = true;
  }
    // if the player touches lava and the player hasn't won
  // Player loses
  if (type == "lava" || type == "laser" && this.status == null) {
    this.status = "lost";
    this.finishDelay = 1;
  } else if (type == "coin") {
